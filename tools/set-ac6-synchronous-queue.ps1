...
$exportName = 'BridgeSynchronousQueueEnabled'
$exportPattern =
...
public static class Ac6SynchronousQueueMemoryNative
{
...
$processHandle = [Ac6SynchronousQueueMemoryNative]::OpenProcess(
...
try {
    ...
    if (-not [Ac6SynchronousQueueMemoryNative]::WriteProcessMemory(
            $processHandle,
            $address,
            $buffer,
    ...
    if (-not [Ac6SynchronousQueueMemoryNative]::ReadProcessMemory(
            $processHandle,
            $address,
            $readBuffer,
            [UIntPtr]::new(4),
    ...
    [void][Ac6SynchronousQueueMemoryNative]::CloseHandle($processHandle)
}
