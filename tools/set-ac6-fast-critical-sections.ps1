...
$exportName = 'BridgeFastCriticalSectionsEnabled'
$exportPattern =
...
public static class Ac6FastCriticalSectionMemoryNative
{
...
$processHandle = [Ac6FastCriticalSectionMemoryNative]::OpenProcess(
...
try {
    ...
    $buffer = [BitConverter]::GetBytes([uint32]$Enabled)
    $bytesWritten = [UIntPtr]::Zero
    if (-not [Ac6FastCriticalSectionMemoryNative]::WriteProcessMemory(
            $processHandle,
            $address,
            $buffer,
    ...
    if (-not [Ac6FastCriticalSectionMemoryNative]::ReadProcessMemory(
            $processHandle,
            $address,
            $readBuffer,
            [UIntPtr]::new(4),
    ...
    [void][Ac6FastCriticalSectionMemoryNative]::CloseHandle($processHandle)
}
